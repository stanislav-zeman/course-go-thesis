...
The course primarily focuses on teaching the Go programming language. However, its goal is to only teach the fundamentals of Go, but the course also focuses on equipping its students with the knowledge often required in practice.

The course builds upon the foundations of an already existing Go Course maintained under RedHat.
...
The last five lectures are completely new and extend the course with numerous technologies and third-party libraries. The original course focused solely on using the standard library. Some of the parts of the standard library, notably the web router, are generally not leveraged in practice. The new lectures give more focus to practical usage; therefore, they are not limited this way.
...
The original RedHat Go course used the Go Present tool. This solution, created by the Go team, implements an exceptional feature: it can run the Go code in your slides and immediately present the output. For solely this reason, I made a decision to continue using it for the new lectures as well as any other markdown-styled solution did not offer any similar viable alternative.
...
The project requirement can be fulfilled in three ways:
\begin{itemize}
